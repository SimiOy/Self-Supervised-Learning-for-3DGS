...
def parse_args():
    ...
    parser.add_argument('--beta', type=float, default=2.0, help='weight for the cross-modality loss')
    ...
    parser.add_argument('--decay_steps', type=int, default=40000,
                        help='decay steps for learning rate')  # Decay every 40,000 steps
    parser.add_argument('--decay_rate', type=float, default=0.1, help='decay rate for learning rate')

    # Data Loader Args
    parser.add_argument('--num_point', type=int, default=2048, help='Point Number')
    parser.add_argument('--use_normals', action='store_true', default=False, help='use normals')
    parser.add_argument('--use_scale_and_rotation', action='store_true', default=False, help='use scale and rotation')
...
def main(args):
    ...
    pointnet = pointnet_model.get_model(normal_channel=args.use_normals)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, img1, img2, img3, y1, y2, y3) in tqdm(enumerate(trainDataLoader, 0),
            ...
            with torch.cuda.amp.autocast():
                fp, trans_feat = pointnet(points)
                fi1 = resnet18(img1)
                fi2 = resnet18(img2)
                fi3 = resnet18(img3)

                hat_y1 = fusion_network(fi1, (fp, trans_feat))
                hat_y2 = fusion_network(fi2, (fp, trans_feat))
                hat_y3 = fusion_network(fi3, (fp, trans_feat))
