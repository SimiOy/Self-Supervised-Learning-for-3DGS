...
class ModelNetDataLoader(Dataset):
    def __init__(self, point_dir, img_dir, args, split='train', opacity_threshold=0.3):
        ...
        self.img_paths = {model_name: os.path.join(img_dir, class_name, split, model_name, 'images')
    ...
    def __getitem__(self, idx):
        ...
        # Load images
        img_dir = self.img_paths[model_name]
        ...
        other_models = [name for name in self.img_paths if name != model_name]
        other_model_name = np.random.choice(other_models)
        img3_dir = self.img_paths[other_model_name]
        ...
        return sampled_properties, img1, img2, img3, y1, y2, y3
    ...
    def visualize_point_cloud(self, index):
        ...
        point_cloud, _, _, _, _, _, _ = self.__getitem__(index)
...
if __name__ == '__main__':
    config = Config(num_point=2048, furthest_point_sample=False, use_normals=False)
    # Usage example:
    ...
    dataset.visualize_point_cloud(65)
